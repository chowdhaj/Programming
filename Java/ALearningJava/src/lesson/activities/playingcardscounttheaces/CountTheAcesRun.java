...
package lesson.activities.playingcardscounttheaces;
...
public class CountTheAcesRun {
   ...
   public static void main(String[] args) { 
      ...
      int seed = 99;
      ArrayList<Card> cards = new ArrayList<Card>(); // Constructor missing the '()'
      Random generator = new Random(seed);
      for (int i = 0; i < 25; i++) {
          cards.add(new Card (generator.nextInt(52), i)); // Added 'new Card' in the constructor // Flipped the order of parameters
      }
      ...
      nines = countCards(cards, Ranks.NINE); // Put this line before 'threes'
      System.out.printf("Nines: %d\n", nines); // Changed parameter 'threes' to 'nines'
      threes = countCards(cards, Ranks.THREE); // Added a semicolon and moved it up a line
      System.out.printf("Threes: %d\n", threes); // Changed parameter 'nines' to 'threes'
      
      
   }
   
   public static void printCards(ArrayList<Card> cards) {
       for (Card c : cards) { // Replaced 'in' with ':'
           System.out.printf("%s ",c); // Added a space after '%s'
       }
       System.out.println();
   }
   ...
   public static int countCards(ArrayList<Card> cards, int rank) {
       int count = 0;
       for (Card c : cards) {
           if (c.rank() == rank) { // Added an extra '=' sign // Added '()' at the end of 'c.rank'
               count++; // Replaced 'count = count++' to 'count++'
           }
       }
       return count;
   }
