...
package lesson.activities.tictactoeboardwinner;

import lesson.activities.tictactoeboardwinner.*;

public class TicTacToeBoardWinnerRun {

    public static void main(String[] args) {
   
        Board b = new Board(new Marker[][] { // Creates a new board and places the following markers on it
                { Marker.X, Marker.X, Marker.X }, // X | X | X
                { Marker.EMPTY, Marker.EMPTY, Marker.EMPTY }, // | | 
                { Marker.O, Marker.O, Marker.O } // 0 | 0 | 0 
        	});
            
        b.Draw(); // Draws the board
        System.out.printf("\n%s is a winner : %s \n",Marker.X, b.Winner(Marker.X)); // Prints out if 'X' is a winner
        System.out.printf("%s is a winner : %s \n",Marker.O, b.Winner(Marker.O)); // Prints out if 'O' is a winner
